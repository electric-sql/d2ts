...
describe('DifferentialDataflow', () => {
  it('should handle basic map and filter operations', () => {
    ...
    for (let i = 0; i < 10; i++) {
      inputWriter.sendData(new Version([0, i]), new MultiSet([[i, 1]]))
      inputWriter.sendFrontier(
  ...
  it('should handle iteration', () => {
    ...
    const geometricSeries = (collection: MultiSet<number>) => {
      return collection
        .map((data) => data * 2)
        .concat(collection)
        .filter((data) => data <= 50)
    ...
    inputWriter.sendData(new Version(0), new MultiSet([[1, 1]]))
    ...
    inputWriter.sendData(new Version(2), new MultiSet([[3, -1]]))
