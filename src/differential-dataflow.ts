...
class DifferenceStreamBuilder<T> {
  ...
  join<K, V1, V2>(
    this: DifferenceStreamBuilder<KeyedData<V1>>,
    other: DifferenceStreamBuilder<KeyedData<V2>>,
  ): DifferenceStreamBuilder<KeyedData<[V1, V2]>> {
...
class GraphBuilder {
  ...
  finalize<T>(): Graph<T> {
    return new Graph(
      this.#streams as DifferenceStreamReader<T>[],
      this.#operators as Operator<T>[],
    )
  }
}

export { DifferenceStreamBuilder, GraphBuilder }
