...
import { DifferenceStreamReader, DifferenceStreamWriter, UnaryOperator, BinaryOperator, MessageType } from './graph';
  ...
  #process(): void {
    for (const { type, data } of this.inputMessages()) {
      if (type === MessageType.DATA) {
        const { version, collection } = data as { version: Version, collection: Collection<T> };
        (this.output as unknown as DifferenceStreamWriter<U>)
          .sendData(version, collection.map(this.#f));
...
export class DebugOperator<T> extends UnaryOperator<T> {
  ...
  #process(): void {
    for (const { type, data } of this.inputMessages()) {
      if (type === MessageType.DATA) {
        const { version, collection } = data as { version: Version, collection: Collection<T> };
        console.log(`debug ${this.#name} data: version: ${version} collection: ${collection}`);
        this.output.sendData(version, collection);
      } else if (type === MessageType.FRONTIER) {
        const frontier = data as unknown as Version;
 ...
 */
export class JoinOperator<K, V1, V2> extends BinaryOperator<KeyedData<V1> | KeyedData<V2>> {
  ...
  constructor(
    ...
    super(
      ...
      output as unknown as DifferenceStreamWriter<KeyedData<V1> | KeyedData<V2>>,
      () => this.#process(),
      initialFrontier
    );
  }
  ...
  #process(): void {
    ...
    for (const { type, data } of this.inputAMessages()) {
      if (type === MessageType.DATA) {
        const { version, collection } = data as unknown as Versioned<KeyedData<V1>>;
    ...
    for (const { type, data } of this.inputBMessages()) {
      if (type === MessageType.DATA) {
        const { version, collection } = data as unknown as Versioned<KeyedData<V2>>;
        for (const [{ key, value }, multiplicity] of collection.getInner()) {
    ...
    const results = new Map<Version, Collection<KeyedData<[V1, V2]>>>();

    for (const [version, collection] of deltaA.join(this.#indexB)) {
      if (!results.has(version)) {
        results.set(version, new Collection());
      }
  ...
  #process(): void {
    for (const { type, data } of this.inputMessages()) {
      if (type === MessageType.DATA) {
        ...
        (this.output as unknown as DifferenceStreamWriter<number>)
          .sendData(version, collection.count());
      } else if (type === MessageType.FRONTIER) {
...
export class ConsolidateOperator<T> extends UnaryOperator<T> {
  #collections = new Map<Version, Collection<T>>();
  ...
  #process(): void {
    for (const { type, data } of this.inputMessages()) {
      if (type === MessageType.DATA) {
        ...
        if (!this.#collections.has(version)) {
          this.#collections.set(version, new Collection());
        }
    ...
    const finishedVersions = Array.from(this.#collections.keys())
      .filter(version => !this.inputFrontier().lessEqual(version));
...
export class FeedbackOperator<T> extends UnaryOperator<T> {
  ...
  #process(): void {
    ...
    for (const elem of elements) {
      ...
      if (this.#inFlightData.has(truncated) && this.#inFlightData.get(truncated)!.size > 0) {
        candidateOutputFrontier.push(elem);

        const closed = new Set(
          Array.from(this.#inFlightData.get(truncated)!).filter(x => x.lessThan(elem))
        );
    ...
    const candidateOutputFrontierAntichain = new Antichain(candidateOutputFrontier);
