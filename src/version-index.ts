...
export class Index<K, V> {
  ...
  join(other: Index<K, V>): [Version, Collection<[K, [V, V]]>][] {
        ...
        for (const [version2, data2] of otherVersions) {
          for (const [val1, mul1] of data1) {
            for (const [val2, mul2] of data2) {
              const resultVersion = version1 instanceof Version && version2 instanceof Version
              ...
              collections.get(resultVersion)!.push([key, [val1, val2], mul1 * mul2]);
            }
          }
        }
      }
    ...
    return Array.from(collections.entries())
      ...
      .map(([version, data]) => [
        version,
        new Collection(data.map(([k, v, m]) => [[k, v], m]))
      ]);
  }
  ...
  compact(compactionFrontier: Antichain, keys: K[] = []): void {
    ...
    const keysToProcess = keys.length > 0 ? keys : Array.from(this.#inner.keys());

    for (const key of keysToProcess) {
      const versions = this.#inner.get(key);
      if (!versions) continue;

      const toCompact = Array.from(versions.keys()).filter(version => 
      ...
      for (const version of toCompact) {
        ...
        const newVersion = version instanceof Version
    ...
    if (this.#compactionFrontier && !this.#compactionFrontier.lessEqual(compactionFrontier)) {
