...
function verifyOrder(results: any[], expectedOrder: string[]) {
  ...
  const sortedByIndex = [...valueToIndex.entries()].sort((a, b) => 
...
describe('fractionalIndexedTopK', () => {
  ...
  it('should handle limit and offset correctly', () => {
    ...
    const initialIds = new Set(initialResult.map(([[_, [value, __]]]) => value.id))
  ...
  it('should handle elements moving positions correctly', () => {
    ...
    const removedValues = new Set(removals.map(([[_, [value, __]]]) => value.value))
    expect(removedValues.has('b')).toBe(true)
    expect(removedValues.has('d')).toBe(true)
    
    // Check that we added 'b+' and 'd+'
    const addedValues = new Set(additions.map(([[_, [value, __]]]) => value.value))
    ...
    const bPlusAddition = additions.find(([[_, [value, __]]]) => value.value === 'b+')
    const dPlusAddition = additions.find(([[_, [value, __]]]) => value.value === 'd+')
